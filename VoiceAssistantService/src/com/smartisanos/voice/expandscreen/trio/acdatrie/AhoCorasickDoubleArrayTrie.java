...
package com.smartisanos.voice.expandscreen.trio.acdatrie;
...
public class AhoCorasickDoubleArrayTrie<V> implements Serializable
{
    ...
    public void prefixSearch(String text, List<Pair<String, V>> hits) {
        hits.clear();
        int currentState = 0;
        int position = 1;
        ...
        for (int i = 0; i < text.length(); ++i)
        {
            ...
            if (hitArray != null) {
                for (int hit : hitArray) {
                    ...
                    if (begin == 0) {
                        Pair<String, V> twoTuple = Pair.of(sb.toString(), v[hit]);
                        hits.add(twoTuple);
                    }
                }
            }
...
//            resize(256 * 32);
        ...
        {
            //@todo for debug only
//            System.out.printf("resize bef: %s bytes\n", RamUsageEstimator.sizeOf(this));
            ...
            used = used2;
            //@todo for debug only
//            System.out.printf("resize aft: %s bytes\n", RamUsageEstimator.sizeOf(this));
...
//                    resize((int) (allocSize * l));
                    // @todo debug only
                    System.out.printf("allocSize: %s, sibling.size :%s, begin: %s, next allocSize: %s\n",
                            allocSize, siblings.size(), begin, siblings.get(siblings.size() - 1).getKey());
                    resize(allocSize + siblings.get(siblings.size() - 1).getKey() + 128);
                }
