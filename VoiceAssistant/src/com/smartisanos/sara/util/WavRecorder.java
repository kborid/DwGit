...
public class WavRecorder {
    ...
    private final static int[] sampleRates = {
            16000, 44100, 22050, 11025, 8000
    };
    public static WavRecorder getInstanse(Boolean recordingCompressed) {
        WavRecorder result = null;

        if (recordingCompressed) {
            result = new WavRecorder(false, AudioSource.CAMCORDER, sampleRates[3],
                    AudioFormat.CHANNEL_CONFIGURATION_MONO, AudioFormat.ENCODING_PCM_16BIT);
        } else {
            int i = 0;
            do {
                result = new WavRecorder(true, AudioSource.CAMCORDER, sampleRates[i],
                        AudioFormat.CHANNEL_CONFIGURATION_MONO, AudioFormat.ENCODING_PCM_16BIT);
     ...
     */
    public WavRecorder(boolean uncompressed, int audioSource, int sampleRate, int channelConfig,
            int audioFormat) {
        try {
