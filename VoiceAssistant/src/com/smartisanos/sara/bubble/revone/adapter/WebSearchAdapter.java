...
public class WebSearchAdapter extends AdapterBase<WebSearchItem> {
    private ISettingBase mSettingManager;

    public WebSearchAdapter(Context context, int[] layoutResArrays) {
        super(context, layoutResArrays);
    }

    public WebSearchAdapter(Context context, int[] layoutResArrays, List<WebSearchItem> data) {
    ...
    @Override
    protected void convert(ViewHolderHelper helper, final WebSearchItem item, final int position) {
        if (item != null) {
            if (!mItemClickable) {
            ...
            helper.setText(R.id.title, item.getEnginName());
